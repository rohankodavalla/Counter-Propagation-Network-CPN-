...
class Counter_Propagation:
    ...
    def generate_weights(self, num_neurons = 1, length=4):
        yy = np.random.rand(num_neurons, length)
        result = np.asarray(yy)
        print("Generating Random weights------", result)
    ...
    def calculate_evklid_way(self, w_vector, x_vector):
        zz= sum([ ((w-x) ** 2) for w, x in zip(w_vector, x_vector)])
        return zz
    ...
    def fit(self, lr_kohonen = 0.35, lr_grossberg = 0.25, epochs = 10):
        print("kohonen_neurons used-----", self.kohonen_neurons, " || grossberg_neurons used------",self.grossberg_neurons )
    ...
    def evaluate(self, X_values, y_values):
        ...
        print("Shortest Euclidean Distance for each Training value------", rr)
        print("Winning node for each Training value------", rr1)


# work neural network
if __name__ == '__main__':
    #print(X_values[0:5],y_values[0:5],y_values[-5:-1])\
    ...
    lrk=0.1 
    lrg= 5
    print("Learning rate for Kohonen ----", lrk , " || Learning rate for Grossberg----" , lrg)
    net.fit(lr_kohonen=1.4, lr_grossberg=0.5, epochs=10)
